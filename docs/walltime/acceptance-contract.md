...
| QC6 | record | invocation membership and order equal the plan's rendered invocations; each `argv_digest` matches, and so does each invocation's **selection identity** — `selector_digest`, `unit_digest` and `atom_digest`, compared against the same identities derived independently from the plan. The three lists (`units`, `selector`, `atoms`) the row also carries are **bound to those digests**: each must digest — under the same empty-list convention the digest is computed with — to the identity beside it. The lists are the plan's, copied in by the assembler, so comparing them against the plan would compare the plan with itself; binding them to the measured digest is what makes them evidence rather than decoration, and what refuses an imported row whose `units` are absent or whose `selector` names another file while every digest is intact. Two name slices of one file differ in exactly the selector and in nothing an argv comparison can see. A plan context carrying no selection identities is a **refusal**, never a pass (R31-F09) |
